...
const Description = ({ company }: { company: string }) => {
  ...
  return (
    ...
    >
      <div className="lists h-full overflow-y-auto overflow-x-hidden  relative z-10">
        <h3 className="text-2xl underline">{company}</h3>
        <p>{title}</p>
        <p>{period}</p>
        <ul className="relative nes-list is-disc">
...
const Experience: React.FC = () => {
  ...
  const bg = "bg-[#f7d51d]";
  return (
    <section className="bg-top bg-cover bg-[url(/Brick.png)] flex flex-col px-0 md:p-8  lg:px-40 lg:pb-40 lg:pt-20 nes-container">
      <div
        className={classNames(
      ...
      <div className="flex p-0 nes-container h-[500px] overflow-hidden bg-transparent">
        <div
          className={classNames(
            " max-w-32 md:max-w-72 xl:max-w-96 relative p-0 border-r-4 border-black -top-1 -bottom-1 h-[101%] ",
            styleColor,
            {
              "": !isOpen,
              "": isOpen,
        ...
        >
          ...
          {resume.experience.map((experience, index: number) => (
            <button
              ...
              className={classNames(
                "nes-btn text-black py-4 px-4 w-full -left-1 z-1 text-xs sm:text-lg overflow-hidden",
                {
                  ["border-r-0 border-r-white selected after:!shadow-[inset,-4px,0px,white] after:!-mr-12"]:
          ...
          {!isOpen && (
            <button
              ...
              className=" nes-btn is-secondary m-3 p-2"
            >
              <FaBars />
            </button>
          )}
